...
int ExpireChannel(struct reggedchannel *chan, struct user *usr_ptr, int preview)
{
  ...
  if (IsSuspended(chan)) {      /* don't remove suspended channels */
...
void docleanupdb(struct user *usr_ptr, char *tail)
{
  ...
  unsigned int chan_ok, chan_expired, chan_empty, chan_owner, chan_suspended, chan_unknown, chan_total;
  ...
  chan_ok = chan_expired = chan_empty = chan_owner = chan_suspended = chan_unknown = 0;
  for (i = 0; i < HASHMAX; i++) {
  ...
  chan_total = chan_expired + chan_empty + chan_owner + chan_suspended + chan_unknown + chan_ok;
  ...
  NoticeToUser(usr_ptr, "%d channel/s still valid (was %d channel/s)",
               (chan_total - (chan_expired + chan_empty + chan_owner)), chan_total);
