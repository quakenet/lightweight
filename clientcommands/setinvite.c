...
void dosetinvite(struct user *user, char *tail)
{
  unsigned char flags;
  ...
  if (channel == NULL) {
    NoticeToUser(user, "Usage: setinvite #channel");
    return;
  }
  ...
  if (!(flags & (CFLAG_MASTER | CFLAG_OWNER)) || IsSuspended(chanptr)) {
    ...
    if (!((user->oper) && (user->authedas->authlevel > 200))) {
      ...
      NoticeToUser(user, "Sorry, you need the +m flag on %s to use setinvite.", channel);
      return;
    }
  }

  /* Do the actual setinvite */
  SetInviteOnly(chanptr);
